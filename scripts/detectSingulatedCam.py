...
class Group:
    ...
    def containsPoint(self, p):
        ...
        for edg in self.edges:
            e = edg[0]
            if squaredDist(p, e) < Group.tolSquared:
                return True
        return False
...
def highlight(mat):
    ...
    for x in range(imgW):
        for y in range(imgH):
            if mat[y][x][2] > 200 and mat[y][x][1] < 100 and mat[y][x][0] < 100:
                p = (x, y)
                adjGroups = []
                nonAdjGroups = []
                for g in groups:
    ...
    colorsT = [(0, 255, 0, 255), (0, 0, 255, 255), (0, 255, 255, 255), (255, 0, 255, 255), (255, 255, 0, 255)]
    it = 0
    for g in groups:
        colNum = it % len(colors)
        for p in g.getPoints():
            x, y = p[0], p[1]
            mat[y][x] = colors[colNum]
        it += 1
    return mat
...
#     pass
# for frame in frames:
#    writer.write(np.abs(frame).astype('u1'))
# writer.release()
